				...
				<div class="tin-1">
					...
					<div class="tin-left">
						...
						<?php
							...
							$row = $c_content->C_get_New_Article($idtheloai);
							extract($row);
							?>
					...
					<div class="tin-right">
					<?php
						$stmt = $c_content->C_get_five_New_Article();
					...
					<?php 
						...
						$total_rows =  $c_content->C_getAllArticle();
					
						$from_record_num = ($records_per_page * $page) ;

						
						$stmt = $c_content->C_get_Articles($from_record_num, $records_per_page);
						while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
							extract($row);
							?>
