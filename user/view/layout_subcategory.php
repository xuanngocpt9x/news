			...
			<?php
				$row = $c_content->C_getOneSubCategory($idloaitin);
				extract($row);
			?>
				<h2><?php echo $Ten; ?></h2>
				...
				<div class="tin-1">
					...
					<div class="tin-left">
						...
						<?php
							...
							$row = $c_content->C_get_New_Article_Sub_Cate($idloaitin);
							extract($row);
							?>
					...
					<div class="tin-right">
					<?php
						$stmt = $c_content->C_get_five_New_Article_Sub_Cate($idloaitin);
					...
					<?php 
						...
						$total_rows =  $c_content->C_count_Article_Sub_Cate($idloaitin);
					
						$from_record_num = ($records_per_page * $page) ;

						
						$stmt = $c_content->C_getArticle_sub_Cate($idloaitin, $from_record_num, $records_per_page);
						while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
							extract($row);
							?>
				...
				<?php
					...
					$page_url = "layout_main.php?p=loaitin&idloaitin=$idloaitin&";
